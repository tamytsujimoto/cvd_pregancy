...
# READING DATASETS
nhanes = readRDS(file = 'Datasets/nhanes_mort_complete.rds')
paq = readRDS(file = 'Datasets/paq_final.rds')
diet = readRDS(file = 'Datasets/diet_final.rds')
...
cvd_partial =
  ...
  mutate(age = RIDAGEYR,
         ...
         flag_educ_hs = ifelse(educ_level >= 3, 1, 0),
         marit_stat = ifelse(DMDMARTL %in% c(77,99), NA, DMDMARTL),
         flag_marit_1 = ifelse(marit_stat == 1, 1, 0),
         hh_size = DMDHHSIZ,
         fam_size = DMDFMSIZ,
         ...
         flag_parity_gt1 = ifelse(n_preg_live > 1, 1, 0),
         ...
         # When n_preg_live = 1 then age at first birth is missing (only available for age_lst_live_brth)
         age_fst_live_brth = ifelse(n_preg_live %in% 1, age_lst_live_brth, age_fst_live_brth),
         ...
         n_infnt_brstfd_1m = ifelse(flag_any_brstfd == 0, 0,
                                 ifelse(RHD230 %in% c(77,99), NA, RHD230)),
         flag_any_brstfd_1m = ifelse(n_infnt_brstfd_1m > 0, 1, 0),
         brstfd = ifelse(flag_any_brstfd == 0, 0, 
                         ifelse(flag_any_brstfd_1m == 0, 1, 2)),
         ...
         time_brst_cancer = age-age_brst_cancer,
         flag_brst_cancer_1y = ifelse(time_brst_cancer > 1, 1, 0),
         flag_brst_cancer_5y = ifelse(time_brst_cancer > 5, 1, 0),
         crp = LBXCRP,
         flag_crp_1 = ifelse(crp > 1, 1, 0),
         bmi = BMXBMI,
         bd_wt_kg = BMXWT,
         bd_ht_cm = BMXHT
  ) %>% 
  #MERGING PAQ AND DIET DATA#
  left_join(paq, by = c('SEQN', 'cycle')) %>% 
  left_join(diet, by = c('SEQN', 'cycle'))
...
cvd_partial =
  ...
  mutate(bpxsy_avg_trt_ln = ifelse(bpxsy_avg_trt == 0, 0, log(bpxsy_avg_trt)),
         ...
         pce_risk_white = ifelse(gender == 1, 
                                 (1-0.9144^exp(pce_score_m_white)), 
                                 (1-0.9665^exp(pce_score_w_white))),
         pce_risk_black = ifelse(gender == 1,
                                 (1-0.8954^exp(pce_score_m_black)),
                                 (1-0.9533^exp(pce_score_w_black))),
...
cvd_partial %>% 
  saveRDS(file = 'Datasets/cvd_partial.rds')

cvd_partial %>% 
  select(SEQN, cycle, SDMVPSU, SDMVSTRA,RIDEXMON,
         bpxsy_avg:flag_subpop_t, eligstat:ucod_leading, pce_risk, pce_risk_cat) %>% 
  saveRDS(file = 'Datasets/cvd_final.rds')

cvd_partial %>% 
  select(SEQN, cycle, SDMVPSU, SDMVSTRA,RIDEXMON,
         bpxsy_avg:flag_subpop_t, eligstat:ucod_leading, pce_risk, pce_risk_cat) %>% 
  write.csv(file = 'Datasets/cvd_final.csv', na = "")
