...
mped = 
  stack_data(path = fped_path, pattern = 'equiv0102', type='sas') %>% 
...
food =
  stack_data(path = nhanes_path, pattern = 'DRXIFF_B') %>% 
  filter(DRDDRSTZ == 1) %>% # reliable dietary recall status #
  mutate(FOODCODE = 1*DRDIFDCD,
         GRMS = DRXIGRMS,
         DAYREC = 0) %>% 
...
nutri = 
  stack_data(path = nhanes_path, pattern = 'DRXTOT_B') %>%
  filter(DRDDRSTZ == 1) %>% # reliable dietary recall status #
  mutate(KCAL = DRXTKCAL,
         ...
         SODI = DRDTSODI,
         DAYREC = 0) %>% 
  select(SEQN, DRDDRSTZ, KCAL, SFAT, SODI, MFAT, PFAT, DAYREC, cycle)
...
compute_hei2015(byid) %>% 
  saveRDS(file = 'Datasets/hei2015_0102.rds')
